...
namespace SLMP {
    ...
    public static class SlmpStruct {
        ...
        /// <summary>
        /// This function converts a C# structure to an array of ushort words for writing to devices.
        /// </summary>
        /// <param name="structObject">The structure object to convert.</param>
        /// <returns>Array of ushort words representing the structure data.</returns>
        /// <exception cref="ArgumentException"></exception>
        public static ushort[] ToWords(object structObject) {
            Type structType = structObject.GetType();
            int structSize = GetStructSize(structType);
            ushort[] words = new ushort[structSize];
            ...
            foreach (var field in fields) {
                ...
                switch (field.FieldType.Name) {
                    case "Int16":
                        words[index] = (ushort)((Int16)(value ?? 0));
                        index++;
                        break;
                    case "UInt16":
                        words[index] = (UInt16)(value ?? 0);
                        index++;
                        break;
                    case "Boolean":
                        words[index] = (ushort)((bool)(value ?? false) ? 1 : 0);
                        index++;
                        break;
                    case "Int32":
                        Int32 int32Val = (Int32)(value ?? 0);
                        words[index] = (ushort)(int32Val & 0xFFFF);
                        words[index + 1] = (ushort)((int32Val >> 16) & 0xFFFF);
                        index += 2;
                        break;
                    case "UInt32":
                        UInt32 uint32Val = (UInt32)(value ?? 0);
                        words[index] = (ushort)(uint32Val & 0xFFFF);
                        words[index + 1] = (ushort)((uint32Val >> 16) & 0xFFFF);
                        index += 2;
                        break;
                    case "String":
                        SlmpStringAttribute? attr = field
                        ...
                        if (stringVal.Length > attr.Length)
                            stringVal = stringVal.Substring(0, attr.Length);
                        stringVal = stringVal.PadRight(attr.Length, '\0');

                        for (int i = 0; i < attr.WordCount; i++) {
                            int charIndex = i * 2;
                            byte lowByte = charIndex < stringVal.Length ? (byte)stringVal[charIndex] : (byte)0;
                            byte highByte = (charIndex + 1) < stringVal.Length ? (byte)stringVal[charIndex + 1] : (byte)0;
                            words[index + i] = (ushort)(lowByte | (highByte << 8));
                        }
                        index += attr.WordCount;
                        break;
                    default:
